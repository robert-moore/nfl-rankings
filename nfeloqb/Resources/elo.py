...
class Elo:
    ...
    def __init__(
        ...
        self.wt_ratings = pd.read_csv(
            "{0}/Manual Data/wt_ratings.csv".format(data_folder)
        )
        ## model variables ##
        self.base = base
        self.k = k
    ...
    def format_games(self):
        ...
        hfa = (
            self.game.groupby(["season"])
            .agg(avg_home_mov=("result", "mean"))
            .reset_index()
        )
        ...
        ## reduce 2020 by 33 ##
        hfa["hfa"] = numpy.where(
            hfa["season"] == 2020, hfa["hfa"] - self.no_fan, hfa["hfa"]
        )
        ## add to games ##
        self.game = pd.merge(
            self.game, hfa[["season", "hfa"]], on=["season"], how="left"
        )
        ## correct for neutrals ##
        self.game["hfa"] = numpy.where(
            self.game["location"] != "Home", 0, self.game["hfa"]
        )
        ## create byes ##
        ...
        ## create by dif ##
        self.game["rest_dif"] = (
            self.game["home_bye"] - self.game["away_bye"]
        ) * self.rest
    ...
    def add_qbs_to_new_games(self):
        ...
        ## add adj ##
        self.game["qb_adj"] = (
            self.game["home_qb_elo_adj"] + self.game["away_qb_elo_adj"]
        )
    ...
    def init_elos(self):
        ...
        self.current_week = self.game[self.game["season"] == self.current_season][
            "week"
        ].min()
    ...
    def calc_elo_difs(self, record):
        ...
        ## calc dif ##
        elo_dif = (
            home_elo - away_elo + record["hfa"] + record["qb_adj"] + record["rest_dif"]
        )
    ...
    def update_elos(self, record, home_elo, away_elo, home_prob, away_prob, elo_dif):
        ...
        mov_mult = numpy.log(abs(record["result"]) + 1) * (
            2.2 / (mov_elo_dif * 0.001 + 2.2)
        )
        ## calc new elos ##
