...
class AirtableWrapper:
    ...
    def __init__(self, model_df, at_config, perform_starter_update=True):
        ...
        self.perform_starter_update = (
            perform_starter_update  ## if True, update starters ##
        )
    ...
    def make_patch_request(self, base, table, headers, data):
        ...
        if resp.status_code != 200:
            print(
                "Error on patch! -- {0} -- {1}".format(resp.status_code, resp.content)
            )
    ...
    def write_chunk(self, base, table, df):
        ...
        self.make_post_request(
            base=base, table=table, headers=self.base_headers, data=data
        )
    ...
    def upsert_chunk(self, base, table, df, upsertFields, key):
        ...
        for field in upsertFields:
            if field not in table_cols:
                print(
                    "     {0} is not included in data. Upsert will fail...".format(
                        field
                    )
                )
        ## iterate through chunk and add to data ##
    ...
    def get_last_starter(self):
        ...
        starters = starters.sort_values(by=["gameday"], ascending=[False]).reset_index(
            drop=True
        )
        ## add a field that combines id and display name ##
        starters["qb_id"] = (
    ...
    def update_qb_table(self):
        ...
        ## get delta ##
        delta = self.all_qbs[
            ~numpy.isin(self.all_qbs["player_id"], self.existing_qbs)
        ].copy()
        ## determine write ##
        if len(delta) > 0:
    ...
    def update_qb_options(self):
        ...
        delta = self.all_qbs[~numpy.isin(self.all_qbs["qb_id"], self.qb_options)].copy()
        ## determine write ##
        if len(delta) > 0:
    ...
    def update_starters(self):
        ...
        if len(writes) > 0:
            ...
            self.write_table(
                base=self.base, table=self.starter_table, df=pd.DataFrame(writes)
            )
        ## update if necessary ##
        if len(updates) > 0:
