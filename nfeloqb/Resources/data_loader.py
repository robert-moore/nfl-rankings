...
class DataLoader:
    ...
    def __init__(self):
        ...
        self.missing_draft_data = "{0}/Manual Data/missing_draft_data.csv".format(
            data_folder
        )
        ## get data on load ##
        self.pull_data()
    ...
    def retrieve_player_stats(self):
        ...
        try:
            df = pd.read_csv(
                self.player_stats_url, compression="gzip", low_memory=False
            )
    ...
    def retrieve_player_meta(self, df):
        ...
        ## get player meta ##
        try:
            meta = pd.read_csv(self.player_info_url)
    ...
    def add_game_data(self, df):
        ## add game data ##
        try:
            game = pd.read_csv(self.game_data_url)
            ## replace team names ##
            ...
            game["game_id"] = (
                game["season"].astype(str)
                + "_"
                + game["week"].astype(str).str.zfill(2)
                + "_"
                + game["away_team"]
                + "_"
                + game["home_team"]
    ...
    def iso_top_passer(self, df):
        ...
        df["is_starter"] = numpy.where(
            df["player_id"] == df["starter_id"], 1, numpy.nan
        )
        return (
            df.sort_values(
                by=["game_id", "is_starter", "attempts"], ascending=[True, False, False]
            )
