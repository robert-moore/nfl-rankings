...
class EloConstructor:
    ...
    def __init__(self, games, qb_model, at_wrapper, elo, export_loc):
        ...
        self.original_elo_file = pd.read_csv(
            self.qb_model.original_file_loc, index_col=0
        )  ## original elo file ##
    ...
    def add_starters(self):
        ...
        def apply_starters(row, starter_dict):
            ...
            row["qb1_value_pre"] = self.extract_starter_values(
                ...
                row["season"],
                row["home_team"],
                starter_dict[row["home_team"]]["draft_number"],
                row["gameday"],
            )
            ...
            row["qb2_value_pre"] = self.extract_starter_values(
                ...
                row["season"],
                row["away_team"],
                starter_dict[row["away_team"]]["draft_number"],
                row["gameday"],
            )
        ...
        self.next_games = self.next_games.apply(
            apply_starters, starter_dict=starter_dict, axis=1
        )
    ...
    def add_team_values(self):
        ...
        ## apply ##
        self.next_games = self.next_games.apply(apply_team_values, axis=1)
    ...
    def create_new_file(self):
        ...
        if self.new_file_games is None:
            ...
            self.new_file = pd.concat(
                [self.original_elo_file, pd.DataFrame(self.new_file_data)]
            )
    ...
    def construct_elo_file(self):
        ...
        if self.new_games is None:
            ...
            print(
                "          Found {0} new games. Adding QB data...".format(
                    len(self.new_games)
                )
            )
        ...
        if self.next_games is None:
            ...
            print(
                "          Found {0} next games. Pulling projected starters...".format(
                    len(self.next_games)
                )
            )
            self.add_starters()
        ...
        if self.new_file_games is None:
            ...
            self.new_file.to_csv("{0}/qb_elos.csv".format(self.export_loc), index=False)
            print("     Done")
