...
class QBModel:
    ...
    def __init__(self, games, model_config):
        ...
        self.league_avg_def = model_config[
            "init_value"
        ]  ## each week, we need to calculate the league avg def for adjs ##
        ...
        self.original_file_loc = "{0}/Manual Data/original_elo_file.csv".format(
            data_folder
        )
        ## initial ##
    ...
    def add_averages(self):
        ...
        ## calc team averages ##
        team_avgs = (
            self.games.groupby(["season", "team"])["team_VALUE"].mean().reset_index()
        )
        ## calc league average ##
        season_avgs = self.games.groupby(["season"])["team_VALUE"].mean().reset_index()
        ## write to stoarge ##
        for index, row in team_avgs.iterrows():
            self.team_avgs["{0}{1}".format(row["season"], row["team"])] = row[
                "team_VALUE"
            ]
    ...
    def get_prev_season_team_avg(self, season, team):
        ...
        return self.team_avgs.get(
            "{0}{1}".format(season - 1, team), self.config["init_value"]
        )
    ...
    def init_qb(self, qb_id, season, team, draft_number, gameday):
        ...
        init_value = min(
            ...
            (
                self.config["rookie_draft_intercept"]
                + (self.config["rookie_draft_slope"] * math.log(draft_number))
            )
            +
    ...
    def handle_qb_regression(self, qb, season):
        ...
        league_regression = league_regression * self.s_curve(
            1, 4, qb["season_starts"], "up"
        )
    ...
    def update_qb_value(self, qb_id, value, gameday, season):
        ...
        ## set rolling sf ##
        rolling_sf = self.config["player_career_sf_base"] + self.s_curve(
        ...
        qb_["rolling_value"] = (
            rolling_sf * value + (1 - rolling_sf) * qb_["rolling_value"]
        )
        ## update variance ##
        ...
        qb_["current_variance"] = (
            self.config["player_sf"]
            * (value - old_value)
            * (value - qb_["current_value"])
    ...
    def handle_team_def_regression(self, team_obj):
        ...
        team_obj["def_value"] = (1 - self.config["team_def_reversion"]) * team_obj[
            "def_value"
    ...
    def handle_team_off_regression(self, team_obj, qb_val, season):
        ...
        team_obj["off_value"] = (
            (
                1
                - self.config["team_off_league_reversion"]
                - self.config["team_off_qb_reversion"]
            )
            * team_obj["off_value"]
            + self.config["team_off_qb_reversion"] * qb_val
            + self.config["team_off_league_reversion"]
            * self.get_prev_season_league_avg(season)
        )
        return team_obj
    ...
    def run_model(self):
        ...
        for index, row in self.games.iterrows():
            ...
            ## get team def value ##
            team_def_val, team_def_adj = self.get_team_def_value(
                row["opponent"], row["week"]
            )
            ## get team off value ##
            team_off_val, team_off_adj = self.get_team_off_value(
                row["team"], qb_val, row["season"]
            )
            ## calc qb adj ##
            ...
            self.update_qb_value(
                row["player_id"], adj_val, row["gameday"], row["season"]
            )
    ...
    def add_elo(self, df):
        ...
        ## dedupe ##
        elo = elo.groupby(["gameday", "team", "opponent", "player_display_name"]).head(
            1
        )
    ...
    def score_model(self, first_season=2009, add_elo=True):
        ...
        if add_elo:
            ...
            record["delta_vs_538"] = (f["f38_se"].mean() ** 0.5) - (
                f["se"].mean() ** 0.5
            )
            ## add rookie model comp ##
            r = f[f["start_number"] <= 10].copy()
            record["delta_vs_538_rookies"] = (r["f38_se"].mean() ** 0.5) - (
                r["se"].mean() ** 0.5
            )
